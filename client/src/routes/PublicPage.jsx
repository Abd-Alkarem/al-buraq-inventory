...
export default function PublicPage() {
  ...
  const fx = (cents) => {
    ...
    return new Intl.NumberFormat(locale, { style: "currency", currency, maximumFractionDigits: 2 }).format(usd * rate);
  };
  ...
  return (
    <div dir={dir} className="min-h-screen bg-neutral-50 relative">
      <BoschBackdrop opacity={0.07} />
      <AnimatedPage>
        ...
        <main className="px-4 md:px-8 py-4">
          <div className="bg-white/90 backdrop-blur rounded-2xl shadow p-4 mb-3 grid gap-2 md:grid-cols-4">
            <input className="border rounded-xl p-2.5 md:col-span-2" placeholder={t.search} value={q} onChange={e=>setQ(e.target.value)} />
          ...
          <div className="overflow-x-auto bg-white/90 backdrop-blur rounded-2xl shadow">
          ...
          {fxUpdated && (
            <div className="text-xs text-gray-400 mt-2">FX updated: {new Date(fxUpdated).toLocaleString()}</div>
          )}
        </main>
      </AnimatedPage>
    </div>
